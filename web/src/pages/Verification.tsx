...
export default function Verification() {
  ...
  // In a real app, the ID would come from the URL params. We'll hardcode 'p1' for demo.
  const demoPolicyId = 'p1';

  useEffect(() => {
    api.verifyPolicy(demoPolicyId)
  ...
  return (
    <div className="flex-1 p-8 max-w-[1000px] mx-auto w-full">
      <div className="mb-8">
        ...
        <p className="text-base text-on-surface-variant mt-1">Immutable cryptographic audit trail for Policy #{demoPolicyId.toUpperCase()}</p>
      </div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 shadow-sm">
          <div className="flex items-center gap-4 mb-8 pb-8 border-b border-outline-variant">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center ${
              verification.is_verified ? 'bg-primary/20 text-primary' : 'bg-error/20 text-error'
            }`}>
              <ShieldCheck size={32} />
            </div>
            <div>
              <h3 className="text-2xl font-bold flex items-center gap-2">
                {verification.is_verified ? 'Cryptographically Verified' : 'Verification Failed'}
                {verification.is_verified && <CheckCircle size={24} className="text-primary" />}
              </h3>
